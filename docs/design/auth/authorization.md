...
### `getUser()` / `getSession()` 使い分け方針

`getSession()` はCookieから取得した値をそのまま返すため改ざんリスクがある。`getUser()` はSupabase Authサーバーへの通信でトークンを再検証するので安全なため、サーバーサイドでは常に `getUser()` を使用する。

| コンテキスト | 使用関数 | 理由 |
|---|---|---|
| Server Actions（各Action冒頭） | `auth.getUser()` | サーバー側での安全な認証チェック |
| Middleware | `auth.getUser()` | セッションの再検証が必要 |
...
`src/middleware.ts` にて認証チェックを行い、未認証ユーザーをログイン画面へリダイレクトする。
...
`deleteAccount` Action（§5.4参照）はSupabase Admin APIを使用するため `service_role` キーが必要となる。取り扱いルールを以下に定める。
