...
`getUser()` はSupabase Authサーバーへの通信でトークンを再検証するため安全だが、ネットワーク呼び出しを伴う。
`getClaims()` はJWT署名をローカルで検証するため、ネットワーク呼び出しなしにトークンを検証できる（新しいプロジェクトではデフォルトで非対称署名キーを使用）。

| コンテキスト | 使用関数 | 理由 |
|---|---|---|
| Server Actions（各Action冒頭） | `auth.getUser()` | サーバー側での安全な認証チェック（ユーザー情報が必要なため） |
| Proxy（`lib/supabase/proxy.ts`） | `auth.getUser()` | セッションのリフレッシュと再検証が必要 |
| ページ・レイアウト保護 | `auth.getClaims()` | ネットワーク不要でJWT検証、ページ保護に推奨 |
