...
export function HomeMemoCard({ memo, onEdit, onFavoriteClick, isPending }: HomeMemoCardProps) {
  return (
    ...
    >
      ...
      {/* メモ内容 */}
      <p className="text-foreground text-sm line-clamp-3 leading-snug">{memo.content}</p>
      ...
      {/* 登録日 */}
      <p className="text-xs text-muted-foreground mt-auto pt-1">{formatDate(memo.created_at)}</p>
    </div>
  )
}
