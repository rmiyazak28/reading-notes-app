...
- **メモ追加の導線**  
  メモの追加は書籍詳細画面（SCR-05）からのみ行う。ホーム・全メモ検索画面にはメモ追加ボタンを置かない。
...
|---|---|
| 実施タイミング | 全機能実装後 |
| 方式 | E2E自動テスト（Playwright）＋シナリオベースのマニュアルテスト（結合テストシナリオ書を別途作成） |
| 対象 | 複数画面・機能にまたがるユーザー操作フロー |

E2Eで自動化するシナリオと、マニュアルテストで補うシナリオを以下のとおり分類する。
...
| 全メモ検索 → タグ絞り込み → お気に入り登録 → お気に入り解除 | F-14 / F-15 |
| 設定画面からプロフィール更新 → ログアウト | SCR-09 |

##### マニュアルテストで補うシナリオ

| シナリオ | 理由 |
|---|---|
| アカウント削除（CASCADE削除の確認） | DBの実データ削除確認は目視が確実 |
| PC / スマホ切り替えを伴うフロー（例：PCでメモ登録 → スマホで検索） | デバイス間のUI差異・レイアウト確認は目視が必要 |
| DBエラー・ネットワーク断時のエラー表示確認 | 異常系の状況再現はE2Eでコストが高い |
