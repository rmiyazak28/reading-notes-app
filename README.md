...
- **認証チェックの使い分け**：Proxy（`src/proxy.ts`）では `getClaims()` による JWT 検証。Server Actions のミューテーション前では `getUser()`（Supabase Auth サーバーへの問い合わせ）で本人確認。`getSession()` はサーバーで使用しない（[詳細](#認証認可の方針)）。
...
- **pg_trgm + GIN インデックス**でタイトル・著者・タグ名の部分一致検索を高速化。メモ検索は `reading_memos.search_text`（content + 書籍タイトル + 著者名 + タグ名を連結した非正規化カラム）への単一 GIN インデックスで横断検索を実現。
...
| バリデーション | Zod | ^4.4.3 |
| 認証 / DB | Supabase（`@supabase/ssr` + `@supabase/supabase-js`） | ^0.10.3 / ^2.106.2 |
| ホスティング | Vercel | — |
...
- **Proxy（`src/proxy.ts`）**：Next.js 16 で `middleware.ts` から改称された Proxy 規約を使用。`lib/supabase/proxy.ts` の `updateSession()` を呼び出し、`getClaims()` による JWT 検証で未認証ユーザーを `/login` へリダイレクト。`matcher` で静的アセットを除外。`getClaims()` は非対称鍵構成では JWKS キャッシュを利用したローカル検証が可能なため、通常は `getUser()` より高速。ただし JWKS 未キャッシュ時や対称鍵構成では Auth サーバーとの通信が発生する。また JWT 有効期限内であれば通過するため、BAN や強制ログアウトのリアルタイム検知はできない。
- **Server Actions**：ミューテーション前に `getUser()`（Supabase Auth サーバーへの問い合わせ）で本人確認。`getClaims()` では検知できないサーバー側セッション無効化に対応するため、信頼境界では必ずネットワーク検証を行う設計とした。
...
| `reading_memos` | 読書メモ | `page_number` は NULL または1以上（CHECK）。`book_id` は `books` を CASCADE 参照。`search_text` は全文横断検索用の非正規化カラム（content + 書籍タイトル + 著者名 + タグ名）でトリガーにより自動同期 |
...
- 部分一致検索用に **pg_trgm + GIN** インデックスを `books.title`/`books.author`/`tags.name` に付与。
- メモの横断検索用に **pg_trgm + GIN** インデックスを `reading_memos.search_text` に付与。

### トリガー

- `books`・`reading_memos` に `updated_at` 自動更新トリガー（`set_updated_at()`）を設定（`tags` は `updated_at` を持たないため対象外）。
- `reading_memos`・`books`・`memo_tags`・`tags` に `search_text` 自動同期トリガーを設定。content・書籍タイトル/著者名・タグ名の変更時に `reading_memos.search_text` を自動再計算する。
...
以下の理由から、単体・マニュアル・E2E・結合の4種類で品質を担保する方針とした。

| 検討した種別 | 採用しなかった理由 |
|---|---|
| コンポーネントテスト | UIロジックを持たない `components/ui/`（shadcn/ui）が中心であり、検証価値に対しテストコストが見合わないため対象外。状態を持つロジックは E2E でカバーする |
| パフォーマンス／負荷テスト | ユーザーごとに RLS でデータが完全分離されており書き込み競合が発生しない構造。インフラのスケーリングは Vercel / Supabase のマネージドサービスに委ねており、現時点では計測対象としていない |
| セキュリティテスト（ペネトレーション等） | RLS の認可境界は E2E の認証フローで間接的に確認。専用ツールでの検証は Semgrep（静的解析）・Betterleaks（シークレットスキャン）で代替し、本番運用規模拡大時に再検討する |
| ビジュアルリグレッション／スナップショットテスト | 開発中の UI 変更頻度が高く、更新コストが検出価値を上回るため見送り。レイアウト崩れはマニュアルテストの目視確認で代替 |
